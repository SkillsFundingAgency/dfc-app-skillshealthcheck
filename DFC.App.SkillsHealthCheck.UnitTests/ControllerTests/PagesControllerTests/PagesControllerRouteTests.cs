...
namespace DFC.App.SkillsHealthCheck.UnitTests.ControllerTests.PagesControllerTests
{
    ...
    public class PagesControllerRouteTests : BasePagesControllerTests
    {
        ...
        {
            new object[] { "/", Guid.Empty, nameof(SkillsHealthCheckController.Index) },
            new object[] { "/pages", Guid.Empty, nameof(SkillsHealthCheckController.Index) },
            new object[] { "/pages/{documentId}", Guid.NewGuid(), nameof(SkillsHealthCheckController.Document) },
        };
        ...
        private static async Task<IActionResult> RunControllerAction(SkillsHealthCheckController controller, Guid documentId, string actionName)
        {
            return actionName switch
            {
                nameof(SkillsHealthCheckController.Document) => await controller.Document(documentId).ConfigureAwait(false),
                _ => await controller.Index().ConfigureAwait(false),
            };
        }

        private SkillsHealthCheckController BuildController(string route)
        {
            var httpContext = new DefaultHttpContext();
            httpContext.Request.Path = route;
            httpContext.Request.Headers[HeaderNames.Accept] = MediaTypeNames.Application.Json;

            return new SkillsHealthCheckController(Logger, FakeMapper, FakeSharedContentItemDocumentService)
            {
